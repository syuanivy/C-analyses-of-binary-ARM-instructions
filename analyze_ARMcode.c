...
unsigned* findData(unsigned *value, unsigned data_addr[]){
   ...
   //printf("ip_offset = %d\n", ip_offset);
    ...
    if(data_addr[0] == 0)
        data_addr[0] = (unsigned) dest;
    else
        data_addr[1] = (unsigned) dest;
    return dest;
}
...
int main(int argc, char **argv){
    ...
    unsigned data_addr[2] ;
    data_addr[0] = 0;
    data_addr[1] = 0;

    //save the address of labels
    unsigned label_addr[6] ;
   int i;
    for ( i = 0; i < 6; i++ ){
      label_addr[i] = 0;
    }

    /* ip is used to iterate through the instruction words.
       bits27_25 dictates the opcode */
    unsigned *ip;
   ...
   //declare the counters
    ...
    /*First pass (1) to get the base address indicated by labels of branches,
      and (2) to get the data section addresses*/
    ...
    for (ip = start; ip <= end; ip++) {
        ...
        if(get_bits(*ip, 24, 4) == 0b1111)
            continue;
        //skip data section
        if(ip == data_addr[0] || ip == data_addr[1])
            continue;
...
void add_block(struct basic_block** phead, struct basic_block** ptail, struct basic_block* new_block)
{   //printf("head->base_address: %p\n", head->base_address);
    //printf("tail->base_address: %p\n", tail->base_address);
    //printf("new_block->base_address: %p\n", new_block->base_address);
    if(*phead == NULL){
        *phead = new_block;
        *ptail = new_block;
    }else{
